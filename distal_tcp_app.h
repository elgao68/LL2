...
#ifndef _DISTAL_TCP_APP_H_
#define _DISTAL_TCP_APP_H_
...
//	float f_Alpha_r;
//	float f_Alpha_p;
} distal_tcp_exercise_targ_params_t;	//Set Target Parameters

typedef struct {
	bool r_brake_disengage;
	bool p_brake_disengage;
} distal_brakes_command_t;

typedef struct {
	float Fr_offset;
	float Tp_offset;
} distal_offset_command_t;
...
typedef struct {
	...
	uint8_t r_brake_status;
} distal_exercise_feedback_params_t;

typedef struct {
	uint8_t index;
	distal_exercise_feedback_params_t distal_exercise_feedback_params[DISTAL_MAX_TARGETS];
} distal_exercise_feedback_info_t;
...
uint8_t set_distal_exercise_feedback_info(float f_x, float f_y, int32_t fQei_L,
		int32_t fQei_R, float fVel_X, float fVel_Y, float fVolt_L, float fVolt_R,
		float fCs_L, float fCs_R, float fFs_X, float fFs_Y, float fCmd_Fx,
		float fCmd_Fy, float fCmd_Fr);
